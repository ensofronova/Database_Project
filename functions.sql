...
create or replace function clear_rating(in business_name text, in feedback text, in grade integer)
  returns void
  language sql
  as $$
...
create or replace function add_supplier(in organisation text, in  reviews  numeric(8,2), in phone_number varchar(11))
	returns void
	language sql
	as $$

	insert into "Suppliers" (organisation, rewiews ,phone_number)
